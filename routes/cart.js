...
try {
    ...
    cart.products.forEach((product) => {
      if (!product || !product.product || !product.product._id) {
        console.error("Invalid product found in cart:", product);
        return; 
    }
        let key = product.product._id.toString(); 
        if (cartDataStructure[key]) {
            cartDataStructure[key].quantity += product.quantity; 
        } else {
            cartDataStructure[key] = {
...
router.get('/add/:id', userIsLoggedIn, async (req, res) => {
  try {
    let cart = await cartModel.findOne({ user: req.session.passport.user });
    let product = await productModel.findOne(req.params.id);
  ...
  if (!cart) {
    ...
    cart = await cartModel.create({
      user: req.session.passport.user,
      products: [{ product: product._id, quantity: 1 }], // Add quantity field
      TotalPrice: Number(product.price),
    });
  } else {
    // Check if the product already exists in the cart
let existingProductIndex = cart.products.findIndex(p => p.product.toString() === product._id.toString());
